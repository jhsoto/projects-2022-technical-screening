...
const altNumbers = (numArray) => {
    // Separate the array into two arrays, one with all positives, one all negatives
    // Retains the order from the original array
    let posNums = [];
    let negNums = [];
    for (let i = 0; i < numArray.length; i++) {
        if (numArray[i] >= 0) {
            posNums.push(numArray[i]);
        } else {
            negNums.push(numArray[i]);
        }
    }

    let outArray = [];
    // Adds an extra negative number to the start of the output if needed
    if (negNums.length > posNums.length) {
        // Add the first number in negNums to the output and remove it from negNums
        outArray.push(negNums.shift());
    }
    // Adds numbers to the output, alternating positive and negative numbers
    // until we run out of negative numbers 
    //     NOTE: extra negatives go at the start which is already handled above
    for (let i = 0; i < negNums.length; i++) {
        outArray.push(posNums[i]);
        outArray.push(negNums[i]);
    }
    // Adds the remaining positive number if it exists
    if (posNums.length > negNums.length) {
        // Add the last number in posNums to the end of output
        outArray.push(posNums.pop());
    }
