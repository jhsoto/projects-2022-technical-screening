...
def is_unlocked(courses_list, target_course):
    ...
    """
    prereqs = CONDITIONS[target_course]
    bracketedItems = re.findall("\((.*)\)", prereqs)
    print(re.findall("\((.*)\)", bracketedItems[0]))

    """needBoth = re.findall("([A-Z]{4}\d{4})\s*and|AND\s*([A-Z]{4}\d{4})", prereqs)
    print(needBoth)
    #equivalents = re.findall("([A-Z]\d{4}\s*(or|OR)\s*)*[A-Z]\d{4}", prereqs)
    equivalents = re.split("\s*[oO][rR]\s*", prereqs)
    """
    print(bracketedItems)
